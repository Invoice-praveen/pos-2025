...
interface InvoiceTemplateProps {
  sale: Sale;
  storeDetails: CompanySettings; // Changed from specific props to CompanySettings type
}
...
export function InvoiceTemplate({ sale, storeDetails }: InvoiceTemplateProps) {
  ...
  return (
    <div className="invoice-container bg-white p-6 sm:p-8 font-sans text-gray-700 text-sm">
      <div className="flex justify-between items-start pb-6 border-b border-gray-200">
        <div className="flex items-center space-x-3">
          {storeDetails.logoUrl ? (
            <img src={storeDetails.logoUrl} alt={`${storeDetails.storeName} Logo`} className="h-12 w-auto max-h-16" data-ai-hint="store logo"/>
          ) : (
            <Store className="h-10 w-10 text-gray-600" />
          )}
          <div>
            <h1 className="text-2xl font-bold text-gray-800">{storeDetails.storeName}</h1>
            {storeDetails.invoiceTagline && <p className="text-xs text-gray-500 uppercase tracking-wider">{storeDetails.invoiceTagline}</p>}
          </div>
        </div>
      ...
      <div className="flex justify-between py-6 border-b border-gray-200">
        ...
        <div className="w-3/5 text-right pl-4">
          ...
          <div className="mt-2 text-xs text-gray-500">
            ...
            {storeDetails.storePhone && <p>{storeDetails.storePhone}{storeDetails.storeEmail && ` | ${storeDetails.storeEmail}`}</p>}
            {!storeDetails.storePhone && storeDetails.storeEmail && <p>{storeDetails.storeEmail}</p>}
          </div>
        </div>
      </div>
      ...
      <div className="py-6">
        {storeDetails.invoiceTerms && (
            <div className="mb-6">
                <h3 className="font-semibold text-gray-700 mb-1">Terms & Conditions</h3>
                <p className="text-gray-500 text-xs">{storeDetails.invoiceTerms}</p>
            </div>
        )}
      ...
      <div className="text-center pt-6 mt-4 border-t border-gray-300">
        <p className="text-gray-600 font-semibold mb-1 uppercase tracking-wider">{storeDetails.receiptFooter || "Thank You For Your Business!"}</p>
        {storeDetails.storeWebsite && <p className="text-gray-500 text-xs">{storeDetails.storeWebsite}</p>}
      </div>
    </div>
  );
}
